...
Transducer: 176 </pre>
<p> As you see, the built-in JavaScript dot method and the transducer gave similar results. The Monad methods are much slower. They're just proof-of-concept demonstrations showing the versatility of monads spawned by Monad(). 


<p>Composition by the dot method and a standard transducer both run faster than the novel Monad() method shown here on arrays of fewer than a million items. My goal is to explore possibilities, not to showcase a better method. When working with small arrays and not many functions, the traditional JavaScript dot method is all you need.</p>
